...
INSERT OR IGNORE INTO notification_types (type_key, name, description, in_app, send_email, email_subject, email_template, is_active, created_at, updated_at) VALUES
    ...
    ('booking_confirmed_client', 'Consulenza confermata (cliente)', 'Al cliente quando la prenotazione è confermata e pagata', 1, 1, '✅ La tua consulenza è confermata', 'booking_confirmed_client.html', 1, datetime('now'), datetime('now')),
    ('booking_cancelled', 'Consulenza annullata', 'All''altro partecipante quando una consulenza viene annullata', 1, 1, '❌ Consulenza annullata', 'booking_cancelled.html', 1, datetime('now'), datetime('now')),
    ('dispute_opened', 'Contestazione aperta', 'Al consulente quando il cliente apre una contestazione su una consulenza', 1, 1, '⚠️ È stata aperta una contestazione su una tua consulenza', 'dispute_opened.html', 1, datetime('now'), datetime('now'));
